...
async function main() {
    fs.writeFileSync('./abi/main/NitroLeague_ABI.json', JSON.stringify(nitroLeague.abi), err => {
        if (err) {
            console.error(err);
        }
    });
    fs.writeFileSync('./abi/main/RaceEvent_ABI.json', JSON.stringify(raceEvent.abi), err => {
        if (err) {
            console.error(err);
        }
    });
    fs.writeFileSync('./abi/main/Race_ABI.json', JSON.stringify(race.abi), err => {
        if (err) {
            console.error(err);
        }
    });

    fs.writeFileSync('./abi/tokens/ERC20Token_ABI.json', JSON.stringify(erc20.abi), err => {
        if (err) {
            console.error(err);
        }
    });
    fs.writeFileSync('./abi/tokens/ERC721NFT_ABI.json', JSON.stringify(erc721.abi), err => {
        if (err) {
            console.error(err);
        }
    });
    fs.writeFileSync('./abi/tokens/ERC1155NFT_ABI.json', JSON.stringify(erc1155.abi), err => {
        if (err) {
            console.error(err);
        }
    });

    fs.writeFileSync('./abi/utils/RaceEventFactory_ABI.json', JSON.stringify(raceEventFactory.abi), err => {
        if (err) {
            console.error(err);
        }
    });
    fs.writeFileSync('./abi/utils/RaceFactory_ABI.json', JSON.stringify(raceFactory.abi), err => {
        if (err) {
            console.error(err);
        }
    });
    fs.writeFileSync('./abi/utils/RewardManager_ABI.json', JSON.stringify(rewardManager.abi), err => {
        if (err) {
            console.error(err);
        }
    });
    fs.writeFileSync('./abi/utils/TokenWithdrawer_ABI.json', JSON.stringify(tokenWithdrawer.abi), err => {
        if (err) {
            console.error(err);
        }
    });
